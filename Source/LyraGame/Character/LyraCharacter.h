...
public:
	...
	/**
	 * 재빙의 등으로 ASC 아바타를 수동 복구했을 때, 이전 아바타(예: 탈 것)에서 남은
	 * Movement.Mode.* 루즈 태그를 정리하고 현재 무브먼트 모드에 맞는 태그로 재설정한다.
	 *
	 * OnAbilitySystemInitialized() 를 통째로 재실행하면 (AC_ShooterCharacter 의 경우)
	 * ShowPawnAfterInitialized() -> AddInitialInventory() 까지 다시 돌아 인벤토리가
	 * 중복 지급되는 부작용이 있어, 태그 정리만 분리해 공개한다.
	 */
	UE_API void RefreshMovementModeGameplayTags();
