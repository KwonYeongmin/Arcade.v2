...
void ALyraCharacter::RefreshMovementModeGameplayTags()
{
	InitializeGameplayTags();
}
...
{
	// Delegate to the movement component so that our custom MaxJumpCount / JumpCount
	// logic (in ULyraCharacterMovementComponent::CanAttemptJump) is respected.
	// ACharacter::JumpIsAllowedInternal also checks JumpCurrentCount < JumpMaxCount,
	// but those are ACharacter's own counters (JumpMaxCount defaults to 1), which
	// would block the 2nd jump even if the movement component allows it.
	if (ULyraCharacterMovementComponent* LyraMovement = Cast<ULyraCharacterMovementComponent>(GetCharacterMovement()))
	{
		return LyraMovement->CanAttemptJump();
	}
