...
ULyraEquipmentInstance* FLyraEquipmentList::AddEntry(TSubclassOf<ULyraEquipmentDefinition> EquipmentDefinition)
{
	...
	// Mover 폰은 B_WeaponInstance_Base 의 애님 레이어 링크 경로를 탈 수 없어(그 매크로가
	// GetTypedPawn(ALyraCharacter) 로 메시를 얻는다) 전용 인스턴스로 대체한다.
	// 계약 이식으로 그 매크로가 폰 타입에 무관해지면 이 분기는 제거 대상이다.
	if (OwnerComponent->GetOwner()->IsA<AAC_CharacterBase>() &&
		InstanceType && InstanceType->IsChildOf(ULyraRangedWeaponInstance::StaticClass()))
	{
		if (UClass* MoverInstanceType = LoadClass<ULyraRangedWeaponInstance>(nullptr,
			TEXT("/Arcade/Weapons/B_WeaponInstance_Pistol.B_WeaponInstance_Pistol_C")))
		{
			InstanceType = MoverInstanceType;
		}
	}
