...
bool ULyraGameplayAbility_Interact::CanActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags, FGameplayTagContainer* OptionalRelevantTags) const
{
	// 상호작용은 플레이어 전용이다. 봇(ALyraPlayerBotController)은 AIController 라
	// PlayerController 가 없는데, 이 어빌리티의 스캔 태스크는 PlayerController 를
	// check() 로 요구한다 (AbilityTask_WaitForInteractableTargets.cpp 의 "//@TODO: Bots?").
	// 어빌리티 셋이 LyraPlayerState 에 부여되면 봇도 함께 받으므로 여기서 걸러낸다.
